...
import cn.codegenerate.client.bo.ProjectFreemarkBO;
import cn.codegenerate.client.domain.CodeColumn;
import cn.codegenerate.client.domain.CodeTable;
import cn.codegenerate.client.error.ProcessException;
import cn.codegenerate.client.utils.FileUtil;
import cn.codegenerate.client.utils.GenerateConstants;
import cn.codegenerate.client.utils.MySqlType;
import cn.codegenerate.client.utils.StringUtill;
...
public class GenerateProcess {
	...
	public boolean polishTableList(List<CodeTable> codeTableList, int databaseType) {
		...
		for (CodeTable codeTable : codeTableList) {
			...
			codeTable.setTableNameFirstlow(StringUtill.firstCharacterLowerCase(codeTable.getTableNamePolished()));
			...
			for (CodeColumn codeColumn : codeTable.getColumns()) {
				...
				//判断这个字段是否存在
				if ("is_delete".equals(codeColumn.getColumnName())) {
					codeTable.setHasDeleteField("1");
				}
				//判断版本
				if ("gmt_create_id".equals(codeColumn.getColumnName())) {
					codeTable.setVersion("2");
				}
			}
...
//		String[] package_paths = projectFreemarkBO.getPackagePath().split("\\.");
		// 包路径加上项目路径
//		package_path = projectFreemarkBO.getUserProjectPath() + File.separator + "src" + File.separator + "main" + File.separator + "java" + File.separator;
//		File p_path = null;
//		// 包文件夹创建
...
//		}
		String[] tempNamePaths = new String[] {"src", "main", "java"};
		// 在项目路径下创建模板对应的文件夹路径
		FileUtil.createDirByPackge(tempNamePaths, projectFreemarkBO.getUserProjectPath());
		String path1 = projectFreemarkBO.getUserProjectPath() + File.separator;
		for (int i = 0; i < tempNamePaths.length; i++) {
			path1 = path1 + tempNamePaths[i] + File.separator;
		}
		//存放java文件的路径
		projectFreemarkBO.setUserProjectPathJava(path1);
		
		tempNamePaths = new String[] {"src", "main", "resources", "mybatis-mappers"};
		// 在项目路径下创建模板对应的文件夹路径
		FileUtil.createDirByPackge(tempNamePaths, projectFreemarkBO.getUserProjectPath());
		path1 = projectFreemarkBO.getUserProjectPath() + File.separator;
		for (int i = 0; i < tempNamePaths.length; i++) {
			path1 = path1 + tempNamePaths[i] + File.separator;
		}
		//存放sqlmapping xml文件的路径
		projectFreemarkBO.setUserProjectPathXml(path1);
	...
	private String getTempleteDestFilePath(String freemarkTempleteName, CodeTable codeTable, ProjectFreemarkBO projectFreemarkBO) {
		...
		// 替换模板文件包路径
		if (replacedTemplete.indexOf("#{package_path}") > -1) {
		...
		replacedTemplete = replacedTemplete.replace("#{table_name_firstlow}", codeTable.getTableNameFirstlow());
		...
		if (tempNamePaths.length >= 3) {
			...
			// 在项目路径下创建模板对应的文件夹路径
			// 判断生成文件格式
			if ("java".equals(getTypeFromTempleteName(freemarkTempleteName))) {
				templeteDestPath = FileUtil.createDirByPackge(tempPath, projectFreemarkBO.getUserProjectPathJava());
			} else {
				templeteDestPath = FileUtil.createDirByPackge(tempPath, projectFreemarkBO.getUserProjectPathXml());
			}
	...
	private String getTypeFromTempleteName(String freemarkTempleteName) {
		if (freemarkTempleteName.endsWith(".xml.ftl")) {
			return "xml";
		}
		return "java";
	}
	private boolean createTempleteDestFile(String templeteDestFilePath, String freemarkTempleteName, CodeTable codeTable, ProjectFreemarkBO projectFreemarkBO, Configuration cfg) {
		...
		data.put("raw_table_cn", codeTable.getTableNameCn());
